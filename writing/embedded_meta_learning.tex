...
For example, if the $z_{old}$ is the system's embedding for trying to win at go, and $z_{meta}$ is the meta-task of switching to trying to lose a game, then $z_{new}$ could be interpreted as the system's guess at a losing strategy for go. We could then evaluate performance by how well the system actually loses at go with the $z_{new}$ strategy. \par
