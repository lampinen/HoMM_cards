...
\item There are also truly held-out tasks that the system has never seen in training. These are the held-out tasks that we evaluate on at the end of training and report results.
\end{enumerate}
This applies analogously to the meta-mappings: each time a meta-mapping is trained, some basic tasks are used as examples while others are held out to encourage generalization. There are also meta-mappings which have never been encountered during training, and those are the meta-mappings we evaluate on at hte end of training. We also evaluate the old (and new) meta-mappings on the new basic tasks that have never been trained.
...
Note that there are three kinds of hold-out in the training of this system, see section 
ref{app_clarifying_holdouts}. \par
