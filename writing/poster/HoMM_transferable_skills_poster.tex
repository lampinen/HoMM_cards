...
\Large
\begin{itemize}
\item Consider a functional perspective. Basic tasks can be seen as functions mapping images to classes, or states to actions, or card hands to bets.
...
\item For example, we could have a try-to-lose function which takes as input a function that tries to win money by betting, and outputs a function which tries to lose money by betting.
\item We suggest that these transformations of functions are a useful way to think about human flexibility.
\end{itemize}
\end{block}
\end{column}
