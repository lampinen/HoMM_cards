...
result_subdirs = c(# "nometa",# "nometa_su_heldout", "nometa_random_holdout_longer", "nometa_only_one",  "nometa_only_one_ho",
                   "basic", "basic_slower_integration", #"basic_su_heldout", 
                   "basic_random_holdout_longer", "basic_random_holdout_longer_slower_integration") # "basic_only_one", "basic_only_one_ho")
...
name_run_type = function(run_type) { # names for plotting
  case_when(run_type == "basic" ~ "Targeted 10% holdout",
            ...
            run_type == "basic_slower_integration" ~ "Targeted 10% holdout",
            run_type == "basic_random_holdout_longer_slower_integration" ~ "Random 50% holdout",
...
optimal_baselines %>%
  select(-game_type) %>%
  mutate_at(vars(expected_reward), function(x) {round(x, digits=3)}) %>%
  stargazer(summary = F, rownames = F, digits=2)
```

```{r}
most_correlated_baselines[,c('run_type','game_type','is_new_game','named_run_type','named_game_type','expected_reward')] %>%
  ungroup() %>%
  filter(run_type %in% c("basic", "basic_random_holdout_longer")) %>%
  select(-game_type, -run_type) %>%
  mutate(is_new_game=as.character(is_new_game)) %>%
  mutate_at(vars(expected_reward), function(x) {round(x, digits=3)}) %>%
  stargazer(summary = F, rownames = F, digits=2)
```
...
avg_reward_d = reward_d %>%
  select(-game_str) %>%
  filter(epoch == 0) %>%
  group_by(named_run_type, run_type, game_type, named_game_type, is_new_game) %>%
  do(data.frame(mean_cl_boot(.$reward))) %>% 
  ungroup() %>%
  rename(average_reward=y, avg_rwd_CI_low=ymin, avg_rwd_CI_high=ymax)
...
avg_reward_d %>%
  ...
  select(-game_type, -run_type) %>%
  mutate(is_new_game=as.character(is_new_game)) %>%
  mutate_at(vars(average_reward, contains("CI")), function(x) {round(x, digits=3)}) %>%
  stargazer(summary = F, rownames = F, digits=2)
```
...
```{r}
summary_plot(reward_d %>%
               filter(run_type %in% c("basic_slower_integration", "basic_random_holdout_longer_slower_integration"),
                      epoch < 500),
             "epoch", "reward", "is_new_game",
             continuous_x=T) +
  facet_wrap(. ~ named_run_type) +
  geom_hline(yintercept=mean(optimal_baselines$expected_reward),
             alpha=0.5) +
...
summary_plot(language_d %>%
               filter(epoch == 0, run_type == "meta_batch_128"),
             "named_game_type", "reward", "is_new_game",
             optimal_baselines) +
  facet_wrap(. ~ named_run_type) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
```{r}
meta_baseline_unmapped_d %>%
  ungroup() %>%
  filter(run_type %in% c("basic", "basic_random_holdout_longer"),
         meta_task != "toggle_suits_rule") %>%
  select(-meta_task, -run_type) %>%
...
meta_d = meta_d %>%
  ...
  mutate(is_new_source = check_if_new_game(run_type, source),
         is_new_target = check_if_new_game(run_type, target),
         is_new =  is_new_source | is_new_target,
         is_new = factor(is_new, labels=c("Trained", "Held out")),
         named_meta_task = name_meta_task(meta_task))
```
...
avg_meta_d = meta_d %>%
  ...
  group_by(run_type, named_run_type, meta_task, named_meta_task, is_new) %>%
  do(data.frame(mean_cl_boot(.$avg_reward))) %>%
  ungroup() %>%
  rename(average_reward=y, avg_rwd_CI_low=ymin, avg_rwd_CI_high=ymax)
...
# Comparing different ways of performing a new task: from examples, from language, or from meta-mappings

```{r}
summary_plot(bind_rows(lang_meta_d %>%
                         filter(epoch == 0,
                                is_new_target & !is_new_source,
                                meta_task == "toggle_losers",
                                run_type == "meta_batch_128") %>%
                         mutate(result_type = "Meta-mapping old task (language cue)") %>%
                         select(result_type, target, run, reward) %>%
                         rename(game_str=target),
                       reward_d %>%                         
                         filter(epoch == 0,
                                is_new_game != "Trained",
                                run_type == "basic") %>%
                         mutate(result_type = "Examples of new task") %>%
                         select(result_type, game_str, run, reward),
                       language_d %>%                         
                         filter(epoch == 0,
                                is_new_game != "Trained",
                                run_type == "meta_batch_128") %>%
                         mutate(result_type = "Language directly") %>%
                         select(result_type, game_str, run, reward),
                       meta_d %>%
                         filter(epoch == 0,
                                is_new_target & !is_new_source,
                                meta_task =="toggle_losers",
                                run_type == "meta_batch_128") %>%
                         mutate(result_type = "Meta-mapping old task (example cue)") %>%
                         select(result_type, target, run, reward) %>%
                         rename(game_str=target)) %>%
               mutate(named_run_type = "Targeted 10% holdout",
                      result_type=str_wrap(result_type, width=20),
                      result_type=factor(result_type, levels=sort(unique(result_type))[c(1,3,4,2)])), # ugh
             "result_type", "reward", "result_type") +
  guides(color=F) +
  labs(x="Performing from ...", y="Average reward on held-out tasks")

#ggsave("~/Documents/grad/res/psych/ARRR/figures/meta_RL/language_meta.png", width=7, height=3.5)
ggsave("writing/figures/why_meta_mapping.png", width=6, height=3)
...
for (result_subdir in c("basic")) {
  for (emb_type in c("final")) {
    ...
    set.seed(0) # reproducible tsne even if result_subdirs change
    ...
    ggplot(ts, aes(x=X1, y=X2, shape=named_game_type, color=losers, size=black_valuable)) +
      ...
      guides(size=guide_legend(title="Switch suits"),
             shape=guide_legend(title="Game"),
             color=guide_legend(title="Losers"))
    ggsave(sprintf("plots/%s_tsne_basic_%s.png", result_subdir, emb_type), width=6, height=4.5)
    ...
    ggplot(pc_d, aes(x=PC1, y=PC2, color=task)) +
      ...
      guides(color=F)
    ggsave(sprintf("plots/%s_PCA_full_%s.png", result_subdir, emb_type), width=8, height=6)
    ...
    ggsave(sprintf("plots/%s_tsne_full_%s.png", result_subdir, emb_type), width=6, height=4)
    }
}
```
