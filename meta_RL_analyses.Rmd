  ...
  if (!is.null(other_baseline_data)) {
    p = p + 
      geom_spoke(data=other_baseline_data,
                 aes(x=game_type, y=expected_reward, angle=0, radius=1),
...
```{r}
ggplot(avg_sweep_d %>%
         filter(run_type == "basic" | run_type == "meta_batch_256"), aes(x=size, color=is_new_game, y=avg_reward)) +
  geom_point() +
  geom_line() +
  geom_errorbar(aes(ymin=ci_lo, ymax=ci_hi)) +
  scale_color_brewer(palette="Set2") +
...
```{r}
ggplot(avg_sweep_d %>% filter(run_type %in% c("basic", "meta_batch_256", "meta_batch_128", "meta_batch_64", "language_mb_128_ne_15"),
...
language_d = load_d(results_dir, c("language_noncompositional_old", 
                                   #"language",  "language_nontoggling_random_holdout", 
                                   "language_simplified", "language_meta_nontoggling", "language_noncompositional",
                                   "language_mb_128_ne_15","language_mb_128_ne_20",
...
lang_meta_d = load_d(results_dir, c(
                              # "language", "language_simplified", "language_meta_nontoggling", "language_nontoggling_random_holdout", 
                               ...
                               "language_mb_128_ne_15","language_mb_128_ne_20",
                               "language_meta_nontoggling", "language_noncompositional"
