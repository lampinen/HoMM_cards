...
```{r}
ggplot(avg_sweep_d %>% filter(run_type %in% c("basic_only_one_ho", "nometa_only_one_ho")), aes(x=size, color=run_type, y=avg_reward)) +
  geom_point() +
  geom_line() +
  geom_errorbar(aes(ymin=ci_lo, ymax=ci_hi)) +
...
```{r}
meta_d = load_d(results_dir, c("basic", "basic_su_heldout", "basic_random_holdout_longer", "basic_only_one", "basic_only_one_ho"), num_runs, "new_meta_true_losses")
